...
/**
 * custom_shell - custom shell implementation that reads user input, interprets and executes commands
...
/**
 * findAndExecCommand - function finds the path to an executable file based on the command-line arguments and then calls fork_cmd to execute the file
 * @content: struct parameter
 *
 * Return: nil
 */
 ...
 * @content: struct parameter
 *
 * Return: -1 (builtin is not found), 0 (builtin is executed), 1 (builtin found but not executed),
...
int search_and_exec_builtin(sh_args *content)
{
	...
	{
		/* If the first argument matches a built-in command, execute it and set the return value */
	...
	/* Return the return value of the executed command or -1 if no command was found */
