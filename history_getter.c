...
/**
 * load_history - reads command history from a file and add it to a history buffer
...
/**
 * write_shel_histry - creates a file to store history of shell program, or appends an existing file
 * @content: struct parameter
 *
...
int write_shel_histry(sh_args *content)
{
	...
	/* Open the history file for writing, truncating it if it exists, with 0644 permissions */
	...
	/* Iterate through the history linked list and write each line to the file */
...
/**
 * add_to_history - builds a history list by adding a new node at the end of the linked list pointed to by 
 * 	content->history
 * @content: struct parameter
 * @buffer: buffer
...
char *get_hist_file_path(sh_args *content)
{
    ...
    /* Calculate the length of the buffer needed to store the path */
	buffer = malloc(sizeof(char) * (len_of_str(home) + len_of_str(HIST_FILE) + 2));
	if (buffer == NULL)
    {
		return (NULL);
    }
...
/**
 * update_hist_node_numbrs - renumbers  the nodes in a linked list of history items
 * @content: struct parameter
 *
 * Return: total number of nodes in the linked list (i.e. the new highest number assigned to a node, plus one)
 */
int update_hist_node_numbrs(sh_args *content)
{
	...
	/* Update the number of nodes in the linked list and return the total number of nodes */
