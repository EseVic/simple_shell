...
{
	int idx = 0;
	/* Loop through the characters of the string until a null character is found */
    for (idx = 0; str[idx] != '\0'; idx++)
	{
		/* Check if the current character matches the target character */
        if (str[idx] == target_char)
		{
			/* If the target character is found, return a pointer to the current character in the string */
            return &str[idx];
		}
	}
	return (NULL);
...
/**
_* string_concat - concatenate a portion of one string, src, onto the end of another string, dest
...
char *string_concat(char *dest, char *src, int max_chars)
{
	...
	/* concatenates characters from the source string to the destination string */
    /* until the end of the source string is reached or the maximum number of */
...
/**
 * cpy_str - copies characters from the source string (src) to the destination string (dest) and ensure that the destination string is null-terminated
 * @dest: destination string
 * @src: source string
...
char *cpy_str(char *dest, char *src, int limit)
{
	...
	/* Copy characters from the source string to the destination string until */
	/* the limit or the end of the source string is reached */
    for ( ; src_idx < limit - 1 && src[src_idx] != '\0'; src_idx++)
        dest[src_idx] = src[src_idx];

	/* If the index of the source string is less than the limit, add null characters */
...
void coments_remover(char *text)
{
    ...
    while (text[idx] != '\0')
    {
		/* Check if the current character is a hash (#) and the previous character is a space */
		/* or if the current character is a hash and it's the first character in the string */
        if ((!idx || text[idx - 1] == ' ') && text[idx] == '#')
        {
			/* Replace the hash with a null terminator to remove the comment */
            text[idx] = '\0';
            break;
        }
        idx++;
...
/**
 * custom_itoa - converts a long integer num into a string representation in a given base, which can be specified through the flags
 * @num: number
...
char *custom_itoa(long int num, int conversionBase, int flags)
{
	...
	/* If the input number is negative and the flag for converting to unsigned is not set */
	...
	/* Choose the conversion array based on the flag for lowercase conversion */
	conversionArray = flags & CONVERT_LOWERCASE ? "0123456789abcdef" : "0123456789ABCDEF";
	
	/* Set the current character pointer to the end of the conversion buffer */
	currentChar = &conversionBuffer[49];
	*currentChar = '\0';

	/* Perform the conversion, starting from the end of the buffer and working backwards */
	do	{
		*--currentChar = conversionArray[unsignedNumber % conversionBase];
		unsignedNumber /= conversionBase;
	} while (unsignedNumber != 0);

	/* If there is a negative sign, add it to the beginning of the buffer */
	if (negativeSign)
	{
