...
/**
 * find_substr_at_start - the string str_to_search starts with the substring substring
...
{
	/* Iterate through the substring until the end of the substring is reached */
	/* while also iterating through the string to search */
    for (; *substring != '\0'; substring++, str_to_search++) {
        
		/* If the current character in the substring doesn't match the current */
		/* character in the string to search, return NULL (no match found) */
...
{
	int idx = 0;
	/* Keep incrementing the index until the end of the string is reached */
	while (str[idx] != '\0')
		idx++;
...
/**
 * cmpare_strs - compare two strings, str1 and str2, and return an integer value indicating their relative order
 * @str1: first string
 * @str2: second string
 *
...
int cmpare_strs(char *str1, char *str2)
{
	...
	{
		/* If the characters at the current position are not equal, return the difference between them */
		if (*str1 != *str2)
			return (*str1 - *str2);
	}

	/* If both strings are equal up to the length of the shorter string, return zero */
	if (*str1 == *str2)
		return (0);
	/* Otherwise, return -1 if str1 is less than str2, or 1 if str1 is greater than str2 */
	else
