 ...
 * Desc: getcwd gets the current working directory and stores it in the buffer. If it is empty, it sets 
 * the dir variable to the value of the HOME environment variable, and if that fails, it sets dir to the 
 * value of the PWD environment variable. It then calls the chdir function to change the current working 
 * directory to the value of dir. If the first argument is not empty, the function checks if it is equal to "-".
 * If it is, it prints the current working directory to standard output and sets the dir variable to the value 
 * of the OLDPWD. If the first argument is not empty and not equal to "-", the function simply calls chdir to
 * change the current working directory to the value of the first argument. Then it sets OLDPWD and PWD to the
...
int change_directory(sh_args *content)
{
    ...
    {
        /* If chdir succeeds, update the environment variables */
...
/**
 * help_command -  to provide a placeholder for the "help" command in a shell program
 * @content: struct parameter
 * 
 * Return: 0
 */
int help_command(sh_args *content)
{
	...
	else
	{
		...
		else if (strcmp(arg_v[1], "help") == 0)
		{
		    ...
		    _puts("Shows help for the specified command, or general help if no command is specified.\n");
        }
 ...
 * Desc: It then checks whether the first character of the input string is a + sign. If it is, it increments 
 * 	 the pointer (str++) to move past the + sign. whether each character is a digit between 0 and 9. If it is, it 
 * 	 multiplies the converted_output by 10 and adds the value of the current digit to it. If the converted_output 
 * 	 exceeds the maximum value of an integer (INT_MAX), the function returns -1 indicating an error
 * 
